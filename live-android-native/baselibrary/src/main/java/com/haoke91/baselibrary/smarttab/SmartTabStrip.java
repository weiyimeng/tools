...
class SmartTabStrip extends LinearLayout {
    ...
    protected void onDraw(Canvas canvas) {
        if (draw) {
            float center = getHeight() - (indicatorThickness / 2f);
            float top = center - (indicatorThickness / 2f);
            float bottom = center + (indicatorThickness / 2f);
            indicatorRectF.set(0, top, getWidth(), bottom);
            indicatorPaint.setColor(Color.parseColor("#F6F6F6"));
    ...
    private void drawIndicator(Canvas canvas, int left, int right, int height, float thickness,
        ...
        if (indicatorWidth == AUTO_WIDTH) {
            ...
            //    indicatorRectF.set(left, top, right, bottom);
            indicatorRectF.set(left + indicatorWidth / 2, top, right - indicatorWidth / 2, bottom);
        }
        ...
        if (indicatorCornerRadius > 0f) {
            ...
            indicatorPaint.setShader(shader);
            canvas.drawRect(indicatorRectF, indicatorPaint);
        }
    }
