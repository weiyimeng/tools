...
//        textView.setTypeface(Typeface.DEFAULT_BOLD);//将tab字体变为细字体
    ...
    private void scrollToTab(int tabIndex, float positionOffset) {
        ...
        if (tabStrip.isIndicatorAlwaysInCenter()) {
            ...
            if (0f < positionOffset && positionOffset < 1f) {
                ...
                extraOffset = Math.round(positionOffset
                        * (selectHalfWidth + nextHalfWidth));
            }
     ...
     * 增加两个tab之间动态设置距离
     *
     * @param textHorizontalPadding
     */
    public void setTabTextHorizontalPadding(int textHorizontalPadding) {
        this.tabViewTextHorizontalPadding = textHorizontalPadding;
    }
