...
public class ThreeVideoTtemLayoutUtils {
        ...
        for (int x = 0; x < movedVideoItems.size(); x++) {
            ...
            if (movedVideoItems.size() < 10) {
                if (movedVideoItems.size() == 8 && x < 2) {
                    ThreeLayoutSizeUilts.videoOldSize(it, rel_wb_container, 2, 3, nameLabelHeight);
                } else {
                    ThreeLayoutSizeUilts.videoOldSize(it, rel_wb_container, 3, 3, nameLabelHeight);
                }
            } else if (movedVideoItems.size() == 10) {
                if (x < 6) {
                    ThreeLayoutSizeUilts.videoOldSize(it, rel_wb_container, 3, 3, nameLabelHeight);
                } else {
                    ThreeLayoutSizeUilts.videoOldSize(it, rel_wb_container, 4, 3, nameLabelHeight);
                }
            } else if (movedVideoItems.size() > 10) {
    ...
    public static void screenOldThreeToSeven(ArrayList<VideoItem> movedVideoItems, RelativeLayout rel_wb_container, double nameLabelHeight) {
        for (int x = 0; x < movedVideoItems.size(); x++) {
            ...
            if (movedVideoItems.size() == 3 && x == 0) {
                ThreeLayoutSizeUilts.videoOldSize(it, rel_wb_container, 2, 1, nameLabelHeight);
            } else if (movedVideoItems.size() == 7 && x < 3) {
                ThreeLayoutSizeUilts.videoSize(it, rel_wb_container, 3, 2, nameLabelHeight);
            } else {
                if (movedVideoItems.size() % 2 == 0) {
                    ThreeLayoutSizeUilts.videoOldSize(it, rel_wb_container, (int) movedVideoItems.size() / 2, 2, nameLabelHeight);
                } else {
                    if (movedVideoItems.size() == 5 && x < 2) {
                        ThreeLayoutSizeUilts.videoOldSize(it, rel_wb_container, 2, 2, nameLabelHeight);
                    } else {
                        ThreeLayoutSizeUilts.videoOldSize(it, rel_wb_container, (int) (movedVideoItems.size() / 2 + 1), 2, nameLabelHeight);
                    }
                }
            }
            RelativeLayout.LayoutParams studentParam = (RelativeLayout.LayoutParams) it.parent.getLayoutParams();
    ...
    public static void screenOldLessThree(ArrayList<VideoItem> movedVideoItems, RelativeLayout rel_wb_container, double nameLabelHeight) {
        for (int i = 0; i < movedVideoItems.size(); i++) {
            ...
            ThreeLayoutSizeUilts.videoOldSize(it, rel_wb_container, movedVideoItems.size(), 1, nameLabelHeight);
            RelativeLayout.LayoutParams studentParam = (RelativeLayout.LayoutParams) it.parent.getLayoutParams();
            studentParam.removeRule(RelativeLayout.ALIGN_PARENT_BOTTOM);
            studentParam.addRule(RelativeLayout.ALIGN_PARENT_TOP);
            studentParam.topMargin = 0;
    ...
    public static void screenThirteen(ArrayList<VideoItem> movedVideoItems, RelativeLayout rel_wb_container, double nameLabelHeight) {
        ...
        if (teacherItem != null) {
            ThreeLayoutSizeUilts.videoSize(teacherItem, rel_wb_container, 4, 3, nameLabelHeight);
        ...
        for (int x = 0; x < withOutTeacherList.size(); x++) {
            ...
            if (x < 4 || x > 7) {
                ThreeLayoutSizeUilts.videoSize(it, rel_wb_container, 4, 3, nameLabelHeight);
            } else {
                ThreeLayoutSizeUilts.videoThirteenSize(it, rel_wb_container, 5, 3, nameLabelHeight);
            }
            RelativeLayout.LayoutParams studentParam = (RelativeLayout.LayoutParams) it.parent.getLayoutParams();
    ...
    public static void screenEightToTwelve(ArrayList<VideoItem> movedVideoItems, RelativeLayout rel_wb_container, double nameLabelHeight) {
        for (int x = 0; x < movedVideoItems.size(); x++) {
            ...
            if (movedVideoItems.size() < 10) {
                if (movedVideoItems.size() == 8 && x < 2) {
                    ThreeLayoutSizeUilts.videoSize(it, rel_wb_container, 2, 3, nameLabelHeight);
                } else {
                    ThreeLayoutSizeUilts.videoSize(it, rel_wb_container, 3, 3, nameLabelHeight);
                }
            } else if (movedVideoItems.size() == 10) {
                if (x < 6) {
                    ThreeLayoutSizeUilts.videoSize(it, rel_wb_container, 3, 3, nameLabelHeight);
    ...
    public static void screenSeven(ArrayList<VideoItem> movedVideoItems, RelativeLayout rel_wb_container, double nameLabelHeight) {
        ...
        if (teacherItem != null) {
            ThreeLayoutSizeUilts.videoSize(teacherItem, rel_wb_container, 3, 2, nameLabelHeight);
            RelativeLayout.LayoutParams teacherParam = (RelativeLayout.LayoutParams) teacherItem.parent.getLayoutParams();
        ...
        for (int x = 0; x < withOutTeacherList.size(); x++) {
            ...
            if (withOutTeacherList.size() == 7) {
                if (x < 3) {
                    ...
                    ThreeLayoutSizeUilts.videoSize(it, rel_wb_container, 4, 2, nameLabelHeight);
                }
            } else {
                if (x < 2) {
                    ThreeLayoutSizeUilts.videoSize(teacherItem, rel_wb_container, 3, 2, nameLabelHeight);
    ...
    public static void screenThreeToSixth(ArrayList<VideoItem> movedVideoItems, RelativeLayout rel_wb_container, double nameLabelHeight) {
        for (int x = 0; x < movedVideoItems.size(); x++) {
            VideoItem it = movedVideoItems.get(x);
            if (movedVideoItems.size() == 3 && x == 0) {
                ThreeLayoutSizeUilts.videoSize(it, rel_wb_container, 2, 1, nameLabelHeight);
            } else {
                if (movedVideoItems.size() % 2 == 0) {
                    ThreeLayoutSizeUilts.videoSize(it, rel_wb_container, (int) movedVideoItems.size() / 2, 2, nameLabelHeight);
                } else {
                    if (movedVideoItems.size() == 5 && x < 2) {
                        ThreeLayoutSizeUilts.videoSize(it, rel_wb_container, 2, 2, nameLabelHeight);
                    } else {
                        ThreeLayoutSizeUilts.videoSize(it, rel_wb_container, (int) (movedVideoItems.size() / 2 + 1), 2, nameLabelHeight);
                    }
                }
            }
            RelativeLayout.LayoutParams studentParam = (RelativeLayout.LayoutParams) it.parent.getLayoutParams();
    ...
    public static void screenLessThree(ArrayList<VideoItem> movedVideoItems, RelativeLayout rel_wb_container, double nameLabelHeight) {
        for (int i = 0; i < movedVideoItems.size(); i++) {
            VideoItem it = movedVideoItems.get(i);
            ThreeLayoutSizeUilts.videoSize(it, rel_wb_container, movedVideoItems.size(), 1, nameLabelHeight);
            RelativeLayout.LayoutParams studentParam = (RelativeLayout.LayoutParams) it.parent.getLayoutParams();
            studentParam.bottomMargin = 0;
            studentParam.topMargin = 0;
            studentParam.removeRule(RelativeLayout.ALIGN_PARENT_TOP);
